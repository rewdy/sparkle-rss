...
const streamSchema = z.string().transform((value): StreamSelector => {
  ...
  if (value.startsWith('feed:')) {
    ...
    if (!Number.isInteger(feedId) || feedId <= 0) throw new AppError(400, 'invalid feed stream');
...
export function createWebApiApp(): Hono<Env> {
  ...
  app.get('/me', async (c) => {
    ...
    return c.json({ userId: user.id, username: user.username, email: user.email });
  });
  ...
  app.post('/folders', async (c) => {
    const body = z.object({ name: z.string().min(1).max(120) }).parse(await c.req.json());
    const s = await getServices();
    return c.json({ folder: await s.folders.create(await userIdOf(s, c), body.name) }, 201);
  });
  ...
  app.get('/subscriptions', async (c) => {
    const s = await getServices();
    return c.json({ subscriptions: await s.subscriptions.list(await userIdOf(s, c)) });
  });
  ...
  app.post('/subscriptions', async (c) => {
    ...
    const s = await getServices();
    const result = await s.subscriptions.subscribe(await userIdOf(s, c), body.url, {
      title: body.title,
      categoryId: body.folderId ?? null,
    });
    return c.json({ subscription: result.subscription, created: result.created }, 201);
  });
  ...
  app.patch('/subscriptions/:feedId', async (c) => {
    ...
    const body = z
      ...
      .parse(await c.req.json());
    const s = await getServices();
    const subscription = await s.subscriptions.edit(await userIdOf(s, c), feedId, {
      title: body.title,
  ...
  app.patch('/entries/read', async (c) => {
    const body = z.object({ ids: idArray, read: z.boolean() }).parse(await c.req.json());
    const s = await getServices();
    return c.json({
      updated: await s.entries.setReadState(await userIdOf(s, c), body.ids, body.read),
    });
  });

  app.patch('/entries/starred', async (c) => {
    const body = z.object({ ids: idArray, starred: z.boolean() }).parse(await c.req.json());
    const s = await getServices();
    return c.json({
      updated: await s.entries.setStarred(await userIdOf(s, c), body.ids, body.starred),
    });
  });
  ...
  app.get('/unread-counts', async (c) => {
    ...
    const folderCounts = subs.reduce<Map<string, number>>((acc, sub) => {
      if (sub.categoryId) {
        acc.set(
          sub.categoryId,
          (acc.get(sub.categoryId) ?? 0) + (byFeed.get(Number(sub.feedId))?.count ?? 0),
        );
      }
      return acc;
    }, new Map());
    return c.json({
      total,
      feeds,
      folders: [...folderCounts.entries()].map(([folderId, count]) => ({ folderId, count })),
    });
  });
  ...
  app.put('/settings', async (c) => {
    ...
    const s = await getServices();
    return c.json({ data: await s.settings.merge(await userIdOf(s, c), body.data) });
  });
  ...
  app.post('/me/api-tokens', async (c) => {
    const body = z
      ...
      .parse(await c.req.json().catch(() => ({})));
    const s = await getServices();
    return c.json(await s.apiTokens.mint(await userIdOf(s, c), body.label), 201);
  });
