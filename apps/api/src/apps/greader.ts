...
async function formArray(body: Record<string, unknown>, key: string): Promise<string[]> {
...
async function parseStreamId(
  ...
  if (value.startsWith('feed/')) {
    ...
    if (Number.isInteger(numeric) && numeric > 0) return { type: 'feed', feedId: numeric };
  ...
  if (value.startsWith(LABEL_PREFIX)) {
    ...
    return id === null ? { type: 'feed', feedId: -1 } : { type: 'folder', categoryId: id };
  }
...
export function createGreaderApp(): Hono<Env> {
  ...
  function checkWriteToken(key: string, userId: string, provided: string): boolean {
  ...
  const auth: MiddlewareHandler<Env> = async (c, next) => {
    ...
    for (const t of tokens) {
      ...
      if (constantTimeEqual(credentials.secret, expected.slice(user.id.length + 1))) {
  ...
  app.onError((error, c) => {
    ...
    console.error(
      JSON.stringify({ level: 'error', msg: 'greader unhandled', err: (error as Error).stack }),
    );
    return c.text('', 500);
  });
  ...
  app.post('/accounts/ClientLogin', async (c) => {
    ...
    const email = typeof form.Email === 'string' ? form.Email : c.req.query('Email');
    const passwd = typeof form.Passwd === 'string' ? form.Passwd : c.req.query('Passwd');
  ...
  app.get('/reader/api/0/user-info', (c) => {
    ...
    return c.json({ userId, userName: userId, userProfileId: userId, userEmail: '' });
  });
      ...
      subscriptions: subs.map((sub) => ({
        ...
        categories:
          sub.categoryId && sub.categoryName
            ? [{ id: `${LABEL_PREFIX}${sub.categoryName}`, label: sub.categoryName }]
            : [],
        url: sub.url,
        htmlUrl: sub.siteUrl,
        iconUrl: sub.iconUrl,
  ...
  app.get('/reader/api/0/unread-count', async (c) => {
    ...
    const unreadcounts = subs.map((sub) => {
      ...
      return {
        ...
        newestItemTimestampUsec: counts ? msToTimestampUsec(counts.newestMs) : '0',
      };
    });
    const folderRollups = new Map<string, { count: number; newestMs: number }>();
    for (const sub of subs) {
      ...
      if (existing) {
        ...
        folderRollups.set(sub.categoryName, { count: counts.count, newestMs: counts.newestMs });
      }
    }
  ...
  async function _resolveLabelToFolder(userId: string, name: string): Promise<number | null> {
    const s = await requireServices();
  ...
  function parseCommonParams(c: Context<Env>): Omit<StreamQuery, 'selector'> {
    ...
    return {
      order,
      limit: Math.min(Math.max(Number.isFinite(limitRaw) ? limitRaw : 20, 1), MAX_STREAM_ITEMS),
  ...
  async function itemEnvelope(
    ...
    return {
      ...
      items: entries.map((entry) => {
        ...
        const categories = ['user/-/state/com.google/reading-list', `feed/${entry.feedId}`];
  ...
  async function handleStreamContents(c: Context<Env>): Promise<Response> {
    ...
    const title = STREAM_TITLES[key] ?? `feed/${(selector as { feedId?: number }).feedId ?? ''}`;
  ...
  app.get('/reader/api/0/stream/items/ids', async (c) => {
    ...
    const params = parseCommonParams(c);
    if (c.req.query('xt') === 'user/-/state/com.google/read') params.unreadOnly = true;
  ...
  app.post('/reader/api/0/stream/items/contents', async (c) => {
    ...
    const userId = c.get('greaderUserId');
    const numeric = ids.map((raw) => parseItemId(raw)).filter((v) => v !== null);
    const s = await requireServices();
  ...
  async function ensureFolderByLabel(userId: string, label: string): Promise<number> {
    const s = await requireServices();
  ...
  function labelFromTag(tag: string): string | null {
    return tag.startsWith(LABEL_PREFIX) ? decodeURIComponent(tag.slice(LABEL_PREFIX.length)) : null;
  }
  ...
  function logSubscription(userId: string, action: string, ref: string): void {
    console.log(
      JSON.stringify({ level: 'info', msg: 'greader subscription', userId, action, ref }),
    );
  }

  app.post('/reader/api/0/subscription/quickadd', async (c) => {
    const body = await c.req.parseBody().catch(() => ({}) as Record<string, unknown>);
    const url = c.req.query('quickadd') ?? (typeof body.quickadd === 'string' ? body.quickadd : '');
    const userId = c.get('greaderUserId');
    const s = await requireServices();
    try {
  ...
  app.post('/reader/api/0/mark-all-as-read', async (c) => {
    ...
    const s = await requireServices();
    const selector = (await parseStreamId(s, userId, String(body.s ?? 'all'))) ?? {
      type: 'all' as const,
    };
  ...
  app.get('/reader/api/0/subscription/export', async (c) => {
    ...
    c.header('Content-Disposition', 'attachment; filename="sparkle-subscriptions.opml"');
    return c.body(xml);
  });
  ...
  app.post('/reader/api/0/subscription/import', async (c) => {
    ...
    for (const item of items.slice(0, 500)) {
      const categoryId =
        item.folderName !== null ? await s.opml.ensureFolderByName(userId, item.folderName) : null;
