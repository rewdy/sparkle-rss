...
describe.skipIf(!process.env.TEST_DATABASE_URL)('greader conformance', () => {
  ...
  beforeAll(async () => {
    ...
    server = createServer((req, res) => {
      if (req.url?.includes('rss')) {
        ...
        res.end('<html><link rel="alternate" type="application/rss+xml" href="/rss"></html>');
      }
    });
  ...
  it('root and compatibility checks answer OK', async () => {
    ...
    expect(await (await app.request('/api/greader.php/check/compatibility')).text()).toBe('OK');
  });
  ...
  it('rejects bad ClientLogin credentials', async () => {
    const res = await app.request('/api/greader.php/accounts/ClientLogin', {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: new URLSearchParams({ Email: DEV_USER, Passwd: 'srk_wrong' }).toString(),
    });
    expect(res.status).toBe(401);
  });

  it('guards reader endpoints behind GoogleLogin auth', async () => {
    expect((await app.request('/api/greader.php/reader/api/0/user-info')).status).toBe(401);
  ...
  it('subscribes over HTTP via discovery (fixture server)', async () => {
    const res = await app.request('/api/greader.php/reader/api/0/subscription/quickadd', {
      method: 'POST',
      headers: { ...authHeader(auth), 'Content-Type': 'application/x-www-form-urlencoded' },
      body: new URLSearchParams({ quickadd: `http://127.0.0.1:${serverPort}/blog` }).toString(),
    });
  ...
  it('stream/items/ids paginates with correct units and filters', async () => {
    ...
    const p2 = (await page2.json()) as { itemRefs: Array<{ id: string }>; continuation?: string };
    expect(p2.itemRefs.map((r) => Number(r.id))).toEqual([entryIdsByGuid.get('c-3')]);
  ...
  it('excludes read items via xt', async () => {
    ...
    await db.update(schema.userEntries).set({ isRead: true }).where(sql`id = ${readId}`);
  ...
  it('hydrates batched items accepting both id forms', async () => {
    ...
    const res = await app.request('/api/greader.php/reader/api/0/stream/items/contents', {
      method: 'POST',
      headers: { ...authHeader(auth), 'Content-Type': 'application/x-www-form-urlencoded' },
      body: (() => {
  ...
  it('toggles star via edit-tag using both tag directions', async () => {
    ...
    const counts = await app.request('/api/greader.php/reader/api/0/unread-count?output=json', {
      headers: authHeader(auth),
    });
    void counts;
  ...
  it('rejects forged write tokens but tolerates empty and x', async () => {
    ...
    const form = new URLSearchParams({ i: '1', a: 'user/-/state/com.google/starred', T: 'bogus' });
  ...
  it('manages folders through rename-tag and disable-tag', async () => {
    ...
    const rename = await app.request('/api/greader.php/reader/api/0/rename-tag', {
      method: 'POST',
      headers: { ...authHeader(auth), 'Content-Type': 'application/x-www-form-urlencoded' },
      body: new URLSearchParams({
    ...
    const tags = await app.request('/api/greader.php/reader/api/0/tag/list?output=json', {
      headers: authHeader(auth),
    });
    const tagBody = (await tags.json()) as { tags: Array<{ id: string }> };
    expect(tagBody.tags.some((t) => t.id === 'user/-/label/NewName')).toBe(true);

    const disable = await app.request('/api/greader.php/reader/api/0/disable-tag', {
      method: 'POST',
      headers: { ...authHeader(auth), 'Content-Type': 'application/x-www-form-urlencoded' },
  ...
  it('mark-all-as-read honors nanosecond bounds', async () => {
    ...
    const res = await app.request('/api/greader.php/reader/api/0/mark-all-as-read', {
      method: 'POST',
      headers: { ...authHeader(auth), 'Content-Type': 'application/x-www-form-urlencoded' },
      body: new URLSearchParams({ s: `feed/${feedId}`, ts: cutoffNs }).toString(),
    });
  ...
  it('round-trips OPML export/import', async () => {
    const exported = await app.request('/api/greader.php/reader/api/0/subscription/export', {
      headers: authHeader(auth),
    });
    ...
    const imported = await app.request('/api/greader.php/reader/api/0/subscription/import', {
      method: 'POST',
      headers: { ...authHeader(auth) },
      body: opmlWithFolder,
    });
    ...
    const subs = await app.request('/api/greader.php/reader/api/0/subscription/list?output=json', {
      headers: authHeader(auth),
    });
    const list = (await subs.json()) as {
      subscriptions: Array<{ url: string; categories: Array<{ id: string }> }>;
    };
    const importedSub = list.subscriptions.find((s) => s.url === 'https://imp.example/f');
  ...
  it('renames a feed via subscription/edit and moves its folder', async () => {
    const edited = await app.request('/api/greader.php/reader/api/0/subscription/edit', {
      method: 'POST',
      headers: { ...authHeader(auth), 'Content-Type': 'application/x-www-form-urlencoded' },
      body: new URLSearchParams({
        ac: 'edit',
    ...
    const list = (await subs.json()) as {
      subscriptions: Array<{ id: string; title: string; categories: Array<{ label: string }> }>;
    };
  ...
  it('unsubscribes via subscription/edit', async () => {
    const res = await app.request('/api/greader.php/reader/api/0/subscription/edit', {
      method: 'POST',
      headers: { ...authHeader(auth), 'Content-Type': 'application/x-www-form-urlencoded' },
      body: new URLSearchParams({ ac: 'unsubscribe', s: `feed/${feedId}` }).toString(),
    });
    expect(await res.text()).toBe('OK');
    const subs = await app.request('/api/greader.php/reader/api/0/subscription/list?output=json', {
      headers: authHeader(auth),
    });
    const list = (await subs.json()) as { subscriptions: Array<{ id: string }> };
    expect(list.subscriptions.find((s) => s.id === `feed/${feedId}`)).toBeUndefined();
  });
});
