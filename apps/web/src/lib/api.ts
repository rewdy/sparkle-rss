...
export const api = {
  ...
  folders: {
    ...
    rename: (id: string, name: string): Promise<void> =>
      raw(`/api/v1/folders/${id}`, {
        method: "PATCH",
        body: JSON.stringify({ name }),
      }).then(assertNoContent),
  ...
  subscriptions: {
    ...
    unsubscribe: (feedId: string): Promise<void> =>
      raw(`/api/v1/subscriptions/${feedId}`, { method: "DELETE" }).then(
        assertNoContent,
      ),
  },
  entries: {
  ...
  tokens: {
    ...
    mint: (label: string): Promise<{ record: { id: string }; token: string }> =>
      request("/api/v1/me/api-tokens", {
        method: "POST",
        body: JSON.stringify({ label }),
      }),
    revoke: (id: string): Promise<void> =>
      raw(`/api/v1/me/api-tokens/${id}`, { method: "DELETE" }).then(
        assertNoContent,
      ),
  },
  opml: {
    ...
    importText: async (
      ...
      const res = await raw("/api/v1/opml/import", {
        method: "POST",
        body: xml,
      });
