...
export function parseRoute(pathname: string): RouteInfo | null {
  ...
  const base = suffix ? pathname.slice(0, pathname.length - suffix[0].length) : pathname;
  const entryId = suffix?.[1] ?? null;
  if (base === '/' || base === '/all') return { stream: { kind: 'all' }, entryId };
  ...
  const feedId = feed?.[1];
  if (feedId !== undefined) return { stream: { kind: 'feed', id: feedId }, entryId };
  const folder = /^\/folder\/(\d+)$/.exec(base);
  const folderId = folder?.[1];
  if (folderId !== undefined) return { stream: { kind: 'folder', id: folderId }, entryId };
  return null;
}
