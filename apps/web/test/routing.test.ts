...
describe('parseRoute', () => {
  it('parses fixed streams', () => {
    expect(parseRoute('/all')).toEqual({ stream: { kind: 'all' }, entryId: null });
    expect(parseRoute('/starred')).toEqual({ stream: { kind: 'starred' }, entryId: null });
    expect(parseRoute('/today')).toEqual({ stream: { kind: 'today' }, entryId: null });
    expect(parseRoute('/unread')).toEqual({ stream: { kind: 'unread' }, entryId: null });
  });

  it('parses parameterized streams', () => {
    expect(parseRoute('/feed/5')).toEqual({ stream: { kind: 'feed', id: '5' }, entryId: null });
  ...
  it('parses entry routes', () => {
    expect(parseRoute('/all/e/123')).toEqual({ stream: { kind: 'all' }, entryId: '123' });
    expect(parseRoute('/feed/5/e/123')).toEqual({
      stream: { kind: 'feed', id: '5' },
      entryId: '123',
    });
    expect(parseRoute('/today/e/9')).toEqual({ stream: { kind: 'today' }, entryId: '9' });
  });
...
describe('date-grouping helpers', () => {
  ...
  it('buckets within today', () => {
    ...
    expect(dayGroup(new Date(2026, 0, 5, 9).getTime(), now)).toBe('January 2026');
  });
  ...
  it('labels today with a wall-clock time', () => {
    expect(timeLabel(new Date(2026, 7, 26, 9, 5).getTime(), now)).toMatch(/9:05/);
  });
});
