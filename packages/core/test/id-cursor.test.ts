...
import { decodeCursor, encodeCursor, parseItemId, toLongItemId, toShortItemId } from '../src/index';
...
describe('stream cursors', () => {
  ...
  it('round-trips starred cursors and enforces expected keys', () => {
    ...
    expect(decodeCursor(token)).toMatchObject({ sortKey: 'starred', primaryAtMs: 5 });
    expect(decodeCursor(token, { sortKey: 'published' })).toBeNull();
  });

  it('accepts legacy published cursors without a sort key', () => {
    const legacy = Buffer.from(JSON.stringify({ p: 42, i: '7', d: 'asc' })).toString('base64url');
