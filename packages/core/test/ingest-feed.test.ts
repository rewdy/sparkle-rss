...
describe("fetchFeed", () => {
  it("sends conditional headers and reports not-modified", async () => {
    ...
    const first = await fetchFeed("https://f.example/rss", {
  ...
  it("follows redirects and flags all-permanent chains", async () => {
    ...
    const result = await fetchFeed("https://old.example/rss", {
      fetchImpl: fetchImpl as never,
    });
    expect(calls).toEqual([
      "https://old.example/rss",
  ...
  it("does not flag temporary redirects as permanent", async () => {
    ...
    const result = await fetchFeed("https://tmp.example/rss", {
      fetchImpl: fetchImpl as never,
    });
