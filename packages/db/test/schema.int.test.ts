...
describe.skipIf(!databaseUrl)('schema integration (docker Postgres)', () => {
  ...
  it('applies migrations and supports the core write path', async () => {
    ...
    await db.insert(schema.users).values({ id: userId, cognitoSub: 'sub-1', username: 'alice' });

    await db.insert(schema.feeds).values({ url: 'https://example.com/feed.xml', title: 'Example' });
    ...
    const guidHash = '5f1b3d2a4c6e8f0a1b2c3d4e5f60718293a4b5c6d7e8f9a0b1c2d3e4f5a6b7c8';
  ...
  it('enforces unique category names per user', async () => {
    const userId = randomUUID();
    await db.insert(schema.users).values({ id: userId, cognitoSub: 'sub-2', username: 'bob' });
    ...
    await db.insert(schema.users).values({ id: otherUser, cognitoSub: 'sub-3', username: 'carol' });
    await expect(
