...
describe.skipIf(!databaseUrl)('core services (docker Postgres)', () => {
  ...
  describe('folders', () => {
    it('creates, lists with counts, renames, deletes', async () => {
      ...
      await expect(folders.create(userId, 'Tech')).rejects.toMatchObject({ status: 409 });
  ...
  describe('subscriptions', () => {
    it('subscribes via discovery, de-dupes feeds, rejects duplicates', async () => {
      ...
      await expect(
        subs.subscribe(userId, 'https://example.org/', { fetch: fetchImpl as never }),
      ).rejects.toMatchObject({ status: 409 });

      const list = await subs.list(userId);
      expect(list.find((s) => s.feedId === result.subscription.feedId)?.url).toBe(
    ...
    it('unsubscribes and removes that user’s entries', async () => {
      ...
      await expect(subs.unsubscribe(userId, feedId)).rejects.toMatchObject({ status: 404 });
    });

    it('edits title and folder', async () => {
      const folders = createFoldersService({ db });
      const folder = await folders.create(userId, 'EditFolder');
      const created = await subs.subscribeDirect(userId, 'https://edit.example/rss.xml', {});
  ...
  describe('entries: pagination, filters, bulk ops', () => {
    ...
    beforeAll(async () => {
      const a = await subs.subscribeDirect(userId, 'https://a.example/rss.xml', {});
      const b = await subs.subscribeDirect(userId, 'https://b.example/rss.xml', {});
    ...
    it('scopes folder streams to member feeds', async () => {
      ...
      const page = await entries.list(userId, { stream: { type: 'folder', categoryId } });
    ...
    it('filters unread and computes unread counts', async () => {
      ...
      expect((await entries.unreadCountsByFeed(userId)).get(feedA)?.count).toBe(5);
    });

    it('stars entries and serves the starred stream by starred_at', async () => {
      const some = await entries.list(userId, { stream: { type: 'all' }, limit: 2 });
      await entries.setStarred(
        userId,
        some.items.map((e) => Number(e.id)),
        true,
      );
      const starred = await entries.list(userId, { stream: { type: 'starred' }, limit: 10 });
    ...
    it('mark-all-read respects stream scope and time bound', async () => {
      ...
      expect((await entries.unreadCountsByFeed(userId)).get(feedB)).toBeUndefined();

      const future = new Date(Date.UTC(2026, 0, 2));
      const bounded = await entries.markAllRead(userId, { type: 'all' }, future);
      expect(bounded).toBeGreaterThan(0);
      const remaining = await entries.unreadCountsByFeed(userId);
      const totalLeft = [...remaining.values()].reduce((s, v) => s + v.count, 0);
      expect(totalLeft).toBeGreaterThan(0);
    });

    it('hydrates items by ids', async () => {
      const page = await entries.list(userId, { stream: { type: 'all' }, limit: 3 });
      ...
      expect(new Set(hydrated.map((h) => h.id))).toEqual(new Set(page.items.map((e) => e.id)));
    });
  });
  ...
  describe('settings + tokens + users', () => {
    ...
    it('provisions users keyed by cognito sub', async () => {
      ...
      const first = await users.ensureByCognitoSub(sub, 'newuser', 'n@example.com');
      const second = await users.ensureByCognitoSub(sub, 'newuser', 'n@example.com');
...
<opml version="2.0"><head><title>t</title></head><body>
  ...
  <outline type="rss" text="Loose" xmlUrl="https://loose.example/feed"/>
      ...
      expect(items[0]).toMatchObject({ folderName: 'News', feedUrl: 'https://a.example/feed' });
      expect(items[1]?.folderName).toBeNull();
    });

    it('round-trips export → import → export', async () => {
      await subs.subscribeDirect(userId, 'https://rt.example/rss.xml', { title: 'RT Feed' });
      ...
      expect(parsed.some((i) => i.feedUrl === 'https://rt.example/rss.xml')).toBe(true);
    });
