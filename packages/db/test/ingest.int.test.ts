...
describe.skipIf(!databaseUrl)("ingest pipeline (docker Postgres)", () => {
  ...
  async function runFeed(
    feedUrl: string,
  ): Promise<{ outcome: string; inserted: number }> {
  ...
  beforeAll(async () => {
    ...
    await db
      .insert(schema.users)
      .values({ id: userId, cognitoSub: "ing-sub", username: "ing" });
  ...
  it("records failures with exponential backoff metadata", async () => {
    ...
    await db
      .update(schema.feeds)
      .set({
        url: "https://int.example/rss.xml",
        errorCount: 0,
        nextFetchAfter: new Date(0),
      })
      .where(sql`id = ${feedId}`);
  });
  ...
  it("fans out only to actual subscribers of the feed", async () => {
    ...
    await db
      .insert(schema.users)
      .values({ id: strangerId, cognitoSub: "x", username: "x" });
