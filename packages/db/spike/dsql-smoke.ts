...
await client.query(`insert into users (id, cognito_sub, username) values ($1,$2,$3)`, [
  userId,
  'smoke-sub',
  'smoke',
]);
...
const feed = await client.query<{ id: string }>(`select id from feeds where url=$1`, [
  'https://example.com/rss.xml',
]);
const feedId = feed.rows[0]?.id;
if (!feedId) throw new Error('feed row missing');
await client.query(`insert into subscriptions (user_id, feed_id) values ($1,$2)`, [userId, feedId]);
await client.query(
...
await client.query(`update user_entries set is_read=true, read_at=now() where user_id=$1`, [
  userId,
]);
