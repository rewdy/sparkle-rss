...
for (const stmt of [
  ...
  const remaining = await pool.query("select tablename from pg_tables where schemaname='public'");
  console.log(`after [${stmt.slice(16, 40)}]: ${remaining.rows.length} tables left`);
}
await dispose();
